...
export async function analyzePOImage(
  ...
  const prompt = 'This is a photograph of a Purchase Order for a metal DEBURRING shop.\nExtract job details and return ONLY a JSON object with these keys:\n  poNumber   - PO number / order number (string)\n  partNumber - part number, item #, or product code (string)\n  quantity   - quantity ordered (number or null)\n  dueDate    - due date / delivery date / ship date formatted as MM/DD/YYYY (string or null)\n  customer   - customer company name (string)\n  notes      - ONLY deburring-relevant instructions: edge break sizes (.005-.015), deburr callouts, blending, finish requirements, Scotch-Brite warnings. Do NOT include material specs, heat treat, certifications, shipping terms, or general notes. Keep it short. (string)\n  priority   - one of "LOW", "NORMAL", "HIGH", "URGENT" based on urgency language like "RUSH", "ASAP", etc.\n\nUse null for any field not found.\nReturn ONLY valid JSON, no markdown fences.';
