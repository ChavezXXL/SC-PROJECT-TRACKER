...
const PrintStyles = () => (
  <style>{`
    @media print {
      ...
      /* New Work Log Print Styles */
      .logs-view-print, .logs-view-print * {
        visibility: visible;
      }
      .logs-view-print {
        position: absolute;
      ...
      .logs-view-print table {
        width: 100%;
        border-collapse: collapse;
        font-size: 14pt; /* BIGGER FONT */
      }
      .logs-view-print th, .logs-view-print td {
      ...
      @page {
        size: landscape; /* Recommend landscape for logs */
        margin: 5mm;
      }
    }
  `}</style>
...
const JobSelectionCard: React.FC<{ job: Job, onStart: (job: Job, op: string) => void, disabled?: boolean, operations: string[] }> = ({ job, onStart, disabled, operations }) => {
...
const EmployeeDashboard = ({ user, addToast, onLogout }: { user: User, addToast: any, onLogout: () => void }) => {
  ...
  // UPDATED: Now passes job details for historical snapshot
  const handleStartJob = async (job: Job, operation: string) => {
    try {
        await DB.startTimeLog(job.id, user.id, user.name, operation, job.partNumber, job.customer);
...
const JobsView = ({ user, addToast, setPrintable, confirm }: any) => {
   ...
   const handleAdminStartJob = async (operation: string) => {
      ...
      try {
          await DB.startTimeLog(startJobModal.id, user.id, user.name, operation, startJobModal.partNumber, startJobModal.customer);
...
const LogsView = ({ addToast }: { addToast: any }) => {
   ...
   const filteredLogs = useMemo(() => {
       ...
       return logs.filter(l => {
           // 1. Status Check
           if (filterStatus === "completed" && !l.endTime) return false;
           if (filterStatus === "in-progress" && l.endTime) return false;

           // 2. Date Range Check (Use endTime for completed jobs context, or startTime if running)
   ...
   }, [logs, filterStatus, dateRange, filterSearch]);

   // Stats
   const totalHours = filteredLogs.reduce((acc, curr) => acc + (curr.durationMinutes || 0), 0) / 60;
      ...
      <div className="space-y-6 logs-view-print">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4 no-print">
            <div className="bg-zinc-900/50 border border-white/5 p-4 rounded-xl">
               <p className="text-zinc-500 text-xs uppercase font-bold">Total Logs</p>
         ...
         <div className="bg-zinc-900 border border-white/10 rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-end md:items-center no-print shadow-sm">
             <div className="flex-1 w-full md:w-auto grid grid-cols-2 md:grid-cols-4 gap-2">
                 ...
                 <div className="flex flex-col gap-1">
                     ...
                     <select value={filterStatus} onChange={(e: any) => setFilterStatus(e.target.value)} className="bg-black/30 border border-white/10 rounded-lg p-2 text-xs text-white">
                 ...
                 <div className="flex items-end">
                      <div className="flex gap-1 w-full">
                         <button onClick={() => setPreset("today")} className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white text-[10px] font-bold py-2 rounded-lg">Today</button>
                         <button onClick={() => setPreset("week")} className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white text-[10px] font-bold py-2 rounded-lg">Week</button>
                         <button onClick={() => setPreset("month")} className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white text-[10px] font-bold py-2 rounded-lg">Month</button>
                      </div>
                 </div>
             </div>
             <div className="w-full md:w-64 relative">
                 <Search className="w-4 h-4 text-zinc-500 absolute left-3 top-2.5"/>
                 <input placeholder="Filter logs..." value={filterSearch} onChange={e => setFilterSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-black/30 border border-white/10 rounded-xl text-sm text-white focus:ring-1 focus:ring-blue-500 outline-none" />
             </div>
         </div>
         ...
         <div className="hidden">
             ...
             <p>Total Hours: {totalHours.toFixed(2)} | Total Jobs: {totalLogs}</p>
             <hr className="my-4"/>
         </div>
         ...
         <div className="bg-zinc-900/50 border border-white/5 rounded-2xl overflow-hidden shadow-sm">
             <div className="max-h-[600px] overflow-y-auto">
                 <table className="w-full text-sm text-left">
                    <thead className="bg-zinc-900 text-zinc-500 sticky top-0 z-10 shadow-sm">
                        <tr>
                            <th className="p-4 font-bold">Date</th>
                            <th className="p-4 font-bold">Time</th>
                            <th className="p-4 font-bold">Job / Part</th>
                    ...
                    <tbody className="divide-y divide-white/5">
                       {filteredLogs.map(l => (
                          <tr key={l.id} className="hover:bg-white/5 transition-colors group">
                             ...
                             <td className="p-4">
                                 <div className="font-bold text-white">{l.jobId}</div>
                                 <div className="text-xs text-zinc-500">{l.partNumber || "No Part #"}</div>
                                 {l.customer && <div className="text-[10px] text-blue-400">{l.customer}</div>}
                             </td>
                             <td className="p-4">
                                 <span className="bg-zinc-800 border border-white/5 px-2 py-1 rounded text-xs text-zinc-300">{l.operation}</span>
                             </td>
                             <td className="p-4 text-white font-medium">{l.userName}</td>
                             <td className="p-4">
                                 {l.endTime ? (
                                     <span className="font-mono text-zinc-300">{formatDuration(l.durationMinutes)}</span>
                                 ) : (
                                     <span className="text-emerald-500 font-bold text-xs animate-pulse">RUNNING</span>
                                 )}
                             </td>
                             <td className="p-4 text-right no-print">
                                 <button onClick={() => handleEditLog(l)} className="p-2 hover:bg-white/10 rounded-lg text-zinc-500 hover:text-white transition-colors"><Edit2 className="w-4 h-4"/></button>
                             </td>
                          </tr>
                       ))}
                       {filteredLogs.length === 0 && <tr><td colSpan={7} className="p-12 text-center text-zinc-500">No logs found for this period.</td></tr>}
                    </tbody>
                 </table>
             </div>
         </div>
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex font-sans">
        ...
        {user.role === "admin" && (
            <aside className="w-64 border-r border-white/5 bg-zinc-950 flex flex-col fixed h-full z-20 no-print">
              ...
              <nav className="px-4 space-y-1">
                  {[
                      ...
                      <button key={x.id} onClick={() => setView(x.id as any)} className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm font-bold ${view === x.id ? "bg-zinc-900 text-white" : "text-zinc-500 hover:text-white"}`}>
        ...
        <main className={`flex-1 p-8 ${user.role === "admin" ? "ml-64 print:ml-0" : ""}`}>
