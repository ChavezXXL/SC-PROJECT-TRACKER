...
export const CommandPalette: React.FC<Props> = ({ open, onClose, onNavigate, jobs, onOpenJob, extraActions = [] }) => {
  ...
  const navItems: CommandItem[] = useMemo(() => [
    ...
    { id: 'nav-customers',   label: 'Customers',    icon: Users,           action: () => onNavigate('admin-customers'), group: 'Navigate', keywords: ['intel', 'concentration', 'quiet', 'revenue'] },
    { id: 'nav-customer-pos',label: 'Customer POs', icon: FileText,        action: () => onNavigate('admin-customer-pos'), group: 'Navigate', keywords: ['po', 'photos', 'purchase order'] },
    { id: 'nav-purchasing',  label: 'Purchasing',   icon: Truck,           action: () => onNavigate('admin-purchase-orders'), group: 'Navigate', keywords: ['vendors', 'outsource', 'po'] },
    { id: 'nav-pricing',     label: 'Pricing',      icon: Calculator,      action: () => onNavigate('admin-pricing'),   group: 'Navigate', keywords: ['price doctor', 'margin', 'cost', 'underpriced'] },
