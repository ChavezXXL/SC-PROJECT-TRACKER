  ...
  const handleConfirmAndCreate = async () => {
    ...
    try {
      ...
      await onJobCreate({
        ...
        info: [editedData.partName ? `Part: ${editedData.partName}` : '', editedData.jobNumber ? `Job#: ${editedData.jobNumber}` : '', editedData.notes || ''].filter(Boolean).join(' | '),
      });
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
          ...
          <div className="p-5 border-t border-gray-700 flex gap-3 flex-shrink-0">
            ...
            <button onClick={handleConfirmAndCreate} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl font-semibold text-sm">
  ...
  if (step === 'error') return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 border border-red-800/50 rounded-2xl w-full max-w-sm shadow-2xl p-8 text-center">
        ...
        <div className="flex gap-3">
          <button onClick={reset} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4" />Try Again</button>
